...
static void config_load(void) {
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        if (strcmp(section, "panel") == 0 && eq) {
            *eq = 0; trim(line);
            char *v = eq + 1; trim(v);
            if (strcmp(line, "font_scale") == 0) g_font_scale = atof(v);
...
static void on_move(GtkButton *b, gpointer ud) {
    int idx = GPOINTER_TO_INT(ud) / 1000;
    int dir = GPOINTER_TO_INT(ud) % 1000; /* +1 down, -1 up */
    int j = idx + dir;
    if (idx < 0 || idx >= g_widget_count) return;
...
static void rebuild_widget_list(void) {
    ...
    for (int i = 0; i < g_widget_count; i++) {
        ...
        g_signal_connect(up, "clicked", G_CALLBACK(on_move), GINT_TO_POINTER(i * 1000 - 1));
        gtk_box_pack_end(GTK_BOX(hbox), up, FALSE, FALSE, 0);
        GtkWidget *down = gtk_button_new_with_label("↓");
        g_signal_connect(down, "clicked", G_CALLBACK(on_move), GINT_TO_POINTER(i * 1000 + 1));
...
int gtk_settings_main(int argc, char **argv) {
    ...
    g_signal_connect(add_btn, "clicked", G_CALLBACK(gtk_menu_popup),
        menu); /* simplified: popup at default position */
