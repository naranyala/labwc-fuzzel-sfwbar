# Lesson: sfwbar Variable Naming Conventions

## Two Types of Variables

sfwbar has two distinct variable contexts that are easy to confuse:
...
When using `ocws-emit` to push variables to sfwbar, the IPC variable names must match what widgets read:
...
4. Test with `sfwbar -R "SetVal VarName = test"` to confirm the variable is reachable
